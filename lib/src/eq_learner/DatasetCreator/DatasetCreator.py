...
from .eq_generator_new import basis_and_symbol_joiner, constant_adder_binomial
...
class DatasetCreator():
    def __init__(self, raw_basis_functions: list(), max_linear_terms: int = 1, max_binomial_terms: int = 0,max_N_terms: int = 0,
        ...
        # #In-elegant solution for the moment, assign to each basis function raised to a certain power (up to order six) a different token: FIX ME
        # possible_polynomial_combinations = []
        # for basis_function in self.basis_functions:
        #     possible_polynomial_combinations.extend(eq_generator_new.N_creator([basis_function],6,1))
        #self.basis_to_token = self.tokenizer(possible_polynomial_combinations) #Create the tokenization. This is needed for having consistent order in the dictionary and assigning to each symbol a proper letter
    ...
    def generate_fun(self):
        ...
        for i in range(n_linear_terms):#range(n_linear_terms):
            singles, raw = eq_generator_new.polynomial_single(tracker,singles,raw, 
        ...
        for i in range(0): #range(n_binomial_terms):
            binomial, priotity_list = eq_generator_new.Binomial_single(total_combinations.binomial,binomial,priotity_list, 
                                                                    self.symbol, constant_interval=self.interval_int)
        ...
        for i in range(n_N_terms):
            N_terms, priotity_list = eq_generator_new.N_single(personalized_basis_fun,N_terms,priotity_list,6)
        ...
        for i in range(n_compositions):
            compositions, raw = eq_generator_new.Composition_single(tracker,compositions, raw,
        ...
        priotity_list = []
        if self.division_on:
            division, priotity_list = eq_generator_new.Division_single(self.basis_functions_to_deprecate,3)
    ...
    def generate_set(self, support, num_equations, isTraining = True, threshold = 2000):
        ...
        dataset = TensorDataset(x_train,y_train_p.long())
        info = self.get_info()
    ...
    # def tokenizer(self,possible_combinations) -> dict():
    #     """Assign a proper token to each symbol. This is needed both for consistency and during neural network training"""
    #     final = {key:idx for idx, key in enumerate(possible_combinations)}
    #     ## Add as well composite entries
    #     for i in range(1,len(self.basis_functions)):
    #         final[self.raw_basis_functions[i-1]] = final[self.basis_functions[i]]
    #     return final